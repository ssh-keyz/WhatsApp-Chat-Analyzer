...
class ChatPreprocessor:
    ...
    def export_to_csv(self):
        ...
        with open('./proc/daily_active_users.csv', 'w', newline='', encoding='utf-8') as file:
        ...
        with open('./proc/total_users_groups.csv', 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(['Total Users', 'Total Groups'])
            writer.writerow([len(self.users), len(self.groups)])

        # Export user interests
        with open('./proc/user_interests.csv', 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
        ...
        # Export events
        with open('./proc/events.csv', 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(['Date', 'Time', 'User', 'Event Description'])
            writer.writerows(self.events)

        print("CSV files exported successfully to ./proc/ directory")
    ...
    def analyze_event_timing(self):
        ...
        with open('./proc/event_timing_analysis.csv', 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
    ...
    def analyze_user_similarity(self):
        ...
        with open('./proc/user_similarity.csv', 'w', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
...
preprocessor.preprocess_chat_to_csv('./chats/_chat2.txt', './proc/formatted_chat.csv')
preprocessor.parse_chat_file('./proc/formatted_chat.csv')
preprocessor.export_to_csv()
preprocessor.analyze_event_timing()
preprocessor.analyze_user_similarity()